...
| `npm run check:integrity` | Passed; 61 tracked or pending files scanned with no NUL corruption |
...
| `npm run test:integration` | Passed: 4 API security cases and 1 full D1/R2/Durable Object integration case |
| `npm run test:e2e` | Passed: 1 two-context Playwright vertical slice in 7.3 seconds |
...
```bash
NODE_OPTIONS=--require=/tmp/ace-omni-network-shim.cjs \
